...
def _send_photo_of_the_day() -> None:
    """Делает запрос к API для получения ссылки на фото дня. Отправляет
    полученное фото во все чаты из таблицы Users2Chats. Удаляет запись из
...
def main() -> None:
    ...
    schedule.every().day.at('14:00').do(_send_photo_of_the_day)
